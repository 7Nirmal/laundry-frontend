...
const Products = () => {
  ...
  const adminGetProducts = () => {
      fetch(`${API}/services/adminGetAllProductsForCategory/${id}`)
      .then((data)=> data.json())
  ...
  const handleChange = (e) => {
    ...
    const handleAddProduct = async() => {
        try{
            const resp = await fetch(`${API}/services/addproducts`, {
            method:'POST',
    ...
    const handleDeleteProduct = async(prodId) => {
        try{
            const resp = await fetch(`${API}/services/adminDeleteProduct/${prodId}`, {
            method:'DELETE',
            headers: { "Content-Type": "application/json"}
                })
                ...
                {products.map((item)=>{
                    return(
                        <div key={item._id} className="single-services-div">
                            ...
                            <div className='action-div'>
                              <button className="edit-prod" onClick={()=>{navigate(`/editProducts/${item._id}`)}}>Edit</button>
